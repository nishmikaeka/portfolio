...
const projects = [
    {
        id: "req",
        title: "ResQ",
        description: "A real-time emergency response platform for Sri Lanka. Victims report SOS with exact location and photos—nearby volunteers respond instantly with one tap, triggering life-saving SMS alerts. Built during recent floods to ensure no one waits alone.",
                ...
                {projects.map((project, index) => (
                    ...
                    >
                        ...
                        <div className="flex flex-1 flex-col gap-4 sm:gap-5 text-left">
                            ...
                            <p className="text-sm sm:text-md tracking-wider text-zinc-600 dark:text-zinc-400">{project.description}</p>

                            <div className="flex flex-wrap gap-3">
                                {project.stack.map((tech) => (
                                    <span
                                        key={tech.name}
                                        className="flex items-center gap-2 rounded-full border border-zinc-200 bg-white px-4 py-1.5 text-sm font-medium text-zinc-700 shadow-sm dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-300"
                                    >
